...
<template>
    <div class="classDetails">
        <v-breadcrumb firstMenu="升级专区" secondMenu="商品内容编辑"></v-breadcrumb>
        <div class="nav row">
            <div class="col-md-12">
                <div class="portlet box red">
                    <div class="portlet-title">
                        <div class="caption">商品内容</div>
                    </div>
                        ...
                        <div class="dialog-con container">
                            <div class="top row">
                                <div class="dialog-item">
                                    ...
                                    <!--分类-->
                                    <div class="col-md-2 ta-right">所属分类：</div>
                                    <div class="col-md-4">{{goodTypeName}}</div>
                                </div>
                            </div>
                            <div class="row">
                                <!--左边-->
                                <div class="left col-md-6">
                                    ...
                                    <div class="dialog-item">
                                        ...
                                        <div class="col-md-5">
                                            ...
                                            <div class="input-group">
                                                <input type="text" class="form-control" placeholder="关联商品或品牌CODE" v-model="dialog.goodsName" />
                                    ...
                                    <!--服务费-->
                                    <div class="dialog-item">
                                        <div class="col-md-4 ta-right">商品服务费：</div>
                                        <div class="col-md-8">
                                            <input type="number" min="0" v-model="dialog.servicePrice" @blur="calcuPrize(1)" />
                                        </div>
                                    </div>
                                    <!--邮费-->
                                    <div class="dialog-item">
                                    ...
                                    <!--详情图-->
                                    <div class="dialog-item row classDetailsPadding">
                                        <div class="col-md-4 ta-right">商品详情图：</div>
                                        <div class="col-md-8">
                                            <div class="lineBlock">
                                                <div class="img-wrap" v-for="item,index in dialog.prodetailImgs">
                                                    <span class="del" @click="handleRemove('prodetailImgs', index)">-</span>
                                                    <input type="file" accept="image/jpeg" @change="handleUpload('prodetailImgs',index)" />
                                                    <img :src="item" />
                                                </div>
                                                <div class="img-add">+
                                                    <input type="file" accept="image/jpeg" @change="handleUpload('prodetailImgs')" />
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                ...
                                <div class="right col-md-6">
                                    ...
                                    <div class="dialog-item">
                                        ...
                                        <div class="col-md-5">
                                            <!-- <el-autocomplete class="inline-input" v-model="dialog.maxGoodName" :fetch-suggestions="_querySearch" placeholder="请输入商品名称" @select="_handleSelect2">
                                            </el-autocomplete> -->
                                            <div class="input-group">
                                                <input type="text" class="form-control" placeholder="关联商品或品牌CODE" v-model="dialog.maxGoodName" />
                                                <span class="input-group-btn">
                                    ...
                                    <!--服务费-->
                                    <div class="dialog-item">
                                        <div class="col-md-4 ta-right">升级商品服务费：</div>
                                        <div class="col-md-8">
                                            <input type="number" min="0" v-model="dialog.maxServicePrice" @blur="calcuPrize(2)" />
                                        </div>
                                    </div>
                                    <!--邮费-->
                                    <div class="dialog-item">
                                    ...
                                    <!--详情图-->
                                    <div class="dialog-item row classDetailsPadding">
                                        <div class="col-md-4 ta-right">升级商品详情图：</div>
                                        <div class="col-md-8">
                                            <div class="lineBlock">
                                                <div class="img-wrap" v-for="item,index in dialog.maxprodetailImgs">
                                                    <span class="del" @click="handleRemove('maxprodetailImgs', index)">-</span>
                                                    <input type="file" accept="image/jpeg" @change="handleUpload('maxprodetailImgs',index)" />
                                                    <img :src="item" />
                                                </div>
                                                <div class="img-add">+
                                                    <input type="file" accept="image/jpeg" @change="handleUpload('maxprodetailImgs')" />
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                            ...
                            <!--概率等杂项-->
                            <div class="bottom row border-shangmian">
                                <div class="dialog-item">
...
export default {
    ...
    methods: {
        ...
        getSelectedGoods(_goods) {
            console.log(_goods, this.goodsListModelType);
            var _item = {
                    id: _goods.proCode,
        ...
        //进入判断是添加还是修改
        addOrEdit(item) {
            this.queryInfo.goodsTypeId = parseInt(this.$route.query.goodsTypeId);
            this.dialog.goodTypeId = this.queryInfo.goodsTypeId;
            this.dialog.goodsType = parseInt(this.$route.query.goodArea); //所属专区
            this.dialog.goodsType == 1 ? this.goodAreaName = '促销专区' : this.goodAreaName = '增值专区';
            this.queryInfo.proCode = this.$route.query.mainId;

            if (this.queryInfo.proCode != undefined) {
                this.conTitle = '修改升级商品';
                this.getDetail();
            } else {
                this.conTitle = '添加升级商品';
            }
        ...
        getGoodsList() {
            ...
            //          if(p == 'goods') {
            //              if(this.dialog.goodName != '') {
            //                  params.proTitle = this.dialog.goodName; //商品标题
            //              }
            //          }
            //          if(p == 'maxGoods') {
            //              if(this.dialog.maxGoodName != '') {
            //                  params.proTitle = this.dialog.maxGoodName; //升级商品标题
            //              }
            //          }
        ...
        calcuPrize(param) {
            if (param == 1) { //商品
                if (this.dialog.oldPrice != undefined &&
                    this.dialog.servicePrice != undefined &&
                    this.dialog.lc != undefined) {
                    // var goodsPrice = parseFloat(this.dialog.oldPrice) + parseFloat(this.dialog.servicePrice) + parseFloat(this.dialog.lc);
                    var goodsPrice = this.NumberAdd(
                        this.NumberAdd(
                            parseFloat(this.dialog.oldPrice),
                            parseFloat(this.dialog.servicePrice)
                        ),
                        parseFloat(this.dialog.lc)
                    );
                    this.$set(this.dialog, 'goodsPrice', goodsPrice);
                }
            } else { //大升级商品
                if (this.dialog.maxOldPrice != undefined && this.dialog.maxServicePrice != undefined &&
                    this.dialog.maxLc != undefined) {
                    // var upgradePrice = parseFloat(this.dialog.maxOldPrice) + parseFloat(this.dialog.maxServicePrice) + parseFloat(this.dialog.maxLc);
                    var upgradePrice = this.NumberAdd(
                        this.NumberAdd(
                            parseFloat(this.dialog.maxOldPrice),
                            parseFloat(this.dialog.maxServicePrice)
                        ),
                        parseFloat(this.dialog.maxLc)
                    );
        ...
        saveData() {
            ...
            this.$http.post(reqUrl, this.dialog).then(function(res) {
                ...
                that.$message(msg);
                var goodArea = that.$route.query.goodArea;
                that.$router.push({
                    path: '/upGoodsList',
                    query: {
                        goodTypeId: that.queryInfo.goodsTypeId,
                        goodArea: goodArea
                    }
                });
            });
        ...
        handleCancle() {
            ...
            this.$router.push({
                path: '/upGoodsList',
                query: {
                    goodTypeId: this.queryInfo.goodsTypeId,
                    goodArea: goodArea
                }
            });
        }
