    ...
    <div id="afterSalesState" :style="{height:bodyHeight}" v-if="asOrder != null">
        <template v-if="asOrder.checkState == '0'">
            ...
            <div class="prompt-box" v-else>
                ...
                <div class="item time">关闭时间:<span>{{ formatDate(asOrder.autoEndTime, 'time') }}</span></div>
            </div>
        </template>
        <template v-if="asOrder.checkState == '1'">
            ...
            <div class="prompt-box" v-if="asOrder.exchangeStatus == '4'">
                <div class="state">
                    ...
                    <template class="val" v-else-if="asOrder.exchangeType == 3">退款</template>
                    成功</div>
                <div class="item time">退款成功时间:<span>{{ formatDate(asOrder.applyTime, 'time') }}</span></div>
        ...
        <template v-if="asOrder.checkState == '2'">
            ...
            <div class="prompt-box" v-else>
                ...
                <div class="item callback">商家拒绝理由:<span>{{ asOrder.refuseReason }}</span></div>
            </div>
        </template>
...
export default {
    ...
    methods: {
        ...
        _getData() {
            ...
            // });
            var res = {
                applyTime: 1523245694000,
                autoEndTime: 1523504894000,
                checkState: 0,
                customerCode: "6ebfe21938d447c498585736b12faac1",
                exchangeCode: "f1df57d8b8a34ee69c566ae74c06422e",
                ...
                exchangeType: 3,
                finalAmount: 0,
                ordersNo: "20180409111514270741",
                productCode: "e70d81da9b374e7f8cc983402fa3c860",
                productName: "雅罗阳光赤霞珠赶你红葡萄酒",
                productSimg: "http://yycmedia.image.alimmdn.com/qqt/WQBBatch_2.4.5/20183301046374214bb62-cf30-4bad-b8e9-761d27e7bdf0.jpg@80w_80h_90Q",
                refundAmount: 588,
                refundPoint: 5880,
                skuNo: "HYC0201010101REDB",
                startTime: 1523245694000,
                updateTime: 1523245694000,
            };
            this.asOrder = res;
        },  
        ...
        _goNegotiate() {
            ...
            this.$router.push({ name: 'afterSalesNegotiate', query: { no: this.pageQuery.no, state: _status } })
        },

        // 修改售后申请
        _goAfterSales() {
            console.log(this.asOrder);
            this.$router.push({
                name: 'afterSales',
                query: {
                    id: this.asOrder.ordersNo
                }
            })
        },
