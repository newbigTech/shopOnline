...
<template>
    <div id="afterSales" :style="{height : bodyHeight}">
        <template v-if="afterSalesShow == true">
            <div ref="AFBox" class="from-box" v-model="dataFrom">
                ...
                <div class="item">
                    <label>退款金额</label>
                    <div class="val">
                        <input ref="amount" @change="_changeAmount" class="amount" type="number" :disabled="dataFrom.amountDisable" :placeholder="dataFrom.amount">
                        <span class="active">|&nbsp;{{dataFrom.finalAmount}}</span>
                        <span class="help">最多¥{{dataFrom.finalAmount}}</span>
                    </div>
                </div>
...
export default {
    ...
    watch: {
        showReceive: function(val, oldVal) {
            this._setHeight();
        }
    },
    destroyed() {
    ...
    methods: {
        ...
        _getOrder() {
            ...
            // console.log(this.order, this.dataFrom);
        },

        // 控制最大退款金额
        _changeAmount(ev) {
            if (parseFloat(this.$refs.amount.value) > parseFloat(this.dataFrom.finalAmount)) { this.$refs.amount.value = this.dataFrom.finalAmount; }
        ...
        // 提交申请
        _action() {
            this.$router.push({name: 'afterSalesState'});
            ...
            //         query: { no: res.exchangeNo, code: res.exchangeCode }
            //     });
            // });
        },
        ...
        // 设置高度
        _setHeight() {
            // if (this.$refs.AFBox == undefined) { return this.FH = 280 + 'px' }
